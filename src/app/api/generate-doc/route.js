...
async function buildDocx(markdown, title, meta = {}) {
  ...
  // Branded running header — the wordmark on every page.
  const header = new Header({
    children: [new Paragraph({
      spacing: { after: 120 },
      children: [new ImageRun({ data: LOGO_BUF, transformation: { width: 118, height: 39 } })],
    })],
  });

  // Footer — a thin rule, tagline left, page number right.
  const footer = new Footer({
    children: [new Paragraph({
      border: { top: { style: BorderStyle.SINGLE, size: 4, color: BRAND.line, space: 6 } },
      tabStops: [{ type: TabStopType.RIGHT, position: TabStopPosition.MAX }],
      children: [
        new TextRun({ text: "Generated by NaviMind — verify against official documentation.", size: 15, color: BRAND.muted }),
        new TextRun({ text: "\t", size: 15 }),
        new TextRun({ text: "Page ", size: 15, color: BRAND.muted }),
        new TextRun({ children: [PageNumber.CURRENT], size: 15, color: BRAND.muted }),
        new TextRun({ text: " of ", size: 15, color: BRAND.muted }),
        new TextRun({ children: [PageNumber.TOTAL_PAGES], size: 15, color: BRAND.muted }),
      ],
    })],
  });

  // Title block: big navy title + optional vessel subtitle + prepared-for/date,
  // closed by a brand-coloured rule.
  const titleBlock = [
    new Paragraph({
      spacing: { after: meta.vessel ? 40 : 80 },
  ...
  if (meta.vessel) {
    titleBlock.push(new Paragraph({
      spacing: { after: 60 },
      children: [new TextRun({ text: String(meta.vessel), size: 20, color: BRAND.muted })],
    }));
  }
  titleBlock.push(new Paragraph({
    spacing: { after: 160 },
    border: { bottom: { style: BorderStyle.SINGLE, size: 12, color: BRAND.navy, space: 6 } },
    tabStops: [{ type: TabStopType.RIGHT, position: TabStopPosition.MAX }],
    children: [
      new TextRun({ text: meta.preparedFor ? `Prepared for: ${meta.preparedFor}` : "", size: 18, color: BRAND.muted }),
      new TextRun({ text: "\t", size: 18 }),
      new TextRun({ text: date, size: 18, color: BRAND.muted }),
    ],
  }));
