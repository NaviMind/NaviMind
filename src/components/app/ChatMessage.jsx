...
function GeneratedDocCard({ file, onOpen }) {
  ...
  return (
    <div className="flex flex-col items-start gap-2 max-w-[340px]">
      {/* The file itself — click to preview. */}
      <button
        onClick={onOpen}
        className="flex items-center gap-2.5 w-full px-3 py-2.5 rounded-xl border border-blue-200 dark:border-blue-500/30 bg-white dark:bg-gray-800/60 shadow-sm text-left hover:bg-blue-50/50 dark:hover:bg-gray-800 transition"
      >
        ...
        <span className="min-w-0">
          ...
          <span className="block text-[10px] font-semibold uppercase tracking-wider text-blue-500 dark:text-blue-400">
            {/(\.pdf$)|(application\/pdf)/i.test(file.name + " " + (file.type || "")) ? "PDF" : "Word"}
          </span>
        </span>
      </button>
