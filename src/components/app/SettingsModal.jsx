...
const IcSun = () => (
  <svg viewBox="0 -960 960 960" fill="currentColor" className="w-[28px] h-[28px]">
      ...
      {THEME_OPTIONS.map(({ value, icon, label }) => {
        ...
        return (
          <button
            ...
            className={`flex-1 flex flex-col items-center justify-center gap-2.5 py-4 px-2 rounded-2xl transition-all duration-200 ring-2
              ${active
                ? "ring-blue-500 bg-blue-50 dark:bg-blue-500/10"
                : "ring-gray-200 dark:ring-white/[0.07] bg-gray-50 dark:bg-white/[0.04] hover:bg-gray-100 dark:hover:bg-white/[0.07]"
              }`}
          >
            <span className={active ? "text-blue-500 dark:text-blue-400" : "text-gray-400 dark:text-gray-500"}>
              {icon}
            </span>
            <span className={`text-[12px] font-medium leading-none ${active ? "text-blue-500 dark:text-blue-400" : "text-gray-500 dark:text-gray-400"}`}>
              {label}
            </span>
          </button>
        );
